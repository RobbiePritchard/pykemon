...
class Pokemon(object):
    ...
    def change_moves(self, move):
        name = raw_input(
            'Trying to learn {} but cant learn more than 4 moves\nwould you like to delete a move to make room for {}: (y)es (n)o '.format(
                move.name, move.name))
        if name == 'y':
            i = 1
    ...
    def __str__(self):
        return "Lv:{}\t{}\t\tHP:{}/{}\tEXP:{}".format(self.level, self.name, self.currentHP, self.hp, self.experience)
